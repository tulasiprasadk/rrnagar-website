...
/**
 * WhatsApp floating icon button (safe version).
 *
 * - DEFAULT_PHONE is digits-only (no '+' sign) to use in wa.me links.
 * - If VITE_WHATSAPP_NUMBER is present it should be digits-only (e.g. 919844007900).
 */

const DEFAULT_PHONE = (import.meta.env && import.meta.env.VITE_WHATSAPP_NUMBER) || "919844007900";
const DEFAULT_MESSAGE = (import.meta.env && import.meta.env.VITE_WHATSAPP_MESSAGE) || "Hi, I need help with RRnagar";
...
export default function WhatsAppFloating({
  ...
  if (hideOn && Array.isArray(hideOn) && hideOn.includes(pathname)) {
    return null;
  }

  // Ensure the phone used in wa.me link is digits-only (no plus)
  const phoneDigits = (phone || "919844007900").toString().replace(/[^0-9]/g, "");
  const encoded = encodeURIComponent(message || "");
  const href = https://wa.me/?text=;
